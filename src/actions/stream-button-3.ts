...
@action({ UUID: "com.saltcannon5k.holo-deck.stream-button-3" })
export class StreamButton3 extends SingletonAction<StreamSettings> {
    override async onWillAppear(
        ev: WillAppearEvent<StreamSettings>
    ): Promise<void> {
        ev.action.setSettings({ id: null });

        const streamData = await readStreamDataFromJson(3);
