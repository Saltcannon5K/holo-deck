...
@action({ UUID: "com.saltcannon5k.holo-deck.stream-button-10" })
export class StreamButton10 extends SingletonAction<StreamSettings> {
    override async onWillAppear(
        ev: WillAppearEvent<StreamSettings>
    ): Promise<void> {
        ev.action.setSettings({ id: null });

        const streamData = await readStreamDataFromJson(10);
