...
@action({ UUID: "com.saltcannon5k.holo-deck.stream-button-1" })
export class StreamButton1 extends SingletonAction<StreamSettings> {
    override async onWillAppear(
        ev: WillAppearEvent<StreamSettings>
    ): Promise<void> {
        ev.action.setSettings({ id: null });

        const streamData = await readStreamDataFromJson(1);
