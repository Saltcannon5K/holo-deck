...
@action({ UUID: "com.saltcannon5k.holo-deck.stream-button-2" })
export class StreamButton2 extends SingletonAction<StreamSettings> {
    override async onWillAppear(
        ev: WillAppearEvent<StreamSettings>
    ): Promise<void> {
        ev.action.setSettings({ id: null });

        const streamData = await readStreamDataFromJson(2);
